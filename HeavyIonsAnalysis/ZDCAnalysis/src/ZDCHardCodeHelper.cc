...
int ZDCHardCodeHelper::rechit_Energy_TriggerBit_EM(const QIE10DataFrame& digi) {
  ...
  double gain = 0;
  if (zside < 0) {
    ped = EM_Peds[0][channel - 1];
    width = EM_PedWidths[0][channel - 1];
    gain = EM_Gains[0][channel - 1];
  } else {
    ped = EM_Peds[1][channel - 1];
    width = EM_PedWidths[1][channel - 1];
    gain = EM_Gains[1][channel - 1];
  }
...
int ZDCHardCodeHelper::rechit_Energy_TriggerBit_HAD(const QIE10DataFrame& digi) {
  ...
  double gain = 0;
  if (zside < 0) {
    ped = HAD_Peds[0][channel - 1];
    width = HAD_PedWidths[0][channel - 1];
    gain = HAD_Gains[0][channel - 1];
  } else {
    ped = HAD_Peds[1][channel - 1];
    width = HAD_PedWidths[1][channel - 1];
    gain = HAD_Gains[1][channel - 1];
  }
  energy = subPedestal(charge(digi, signalTs_), ped, width) * gain;
  noise = ootpuFrac * subPedestal(charge(digi, noiseTs_), ped, width) * gain;
  int int_Energy = std::min(std::max(0, int(energy / (50.0))), 1023);
...
double ZDCHardCodeHelper::rechit_Energy_RPD(const QIE10DataFrame& digi) {
  ...
  double gain = 0;
  if (zside < 0) {
    ped = RPD_Peds[0][channel - 1];
    width = RPD_PedWidths[0][channel - 1];
    gain = RPD_Gains[0][channel - 1];
  } else {
    ped = RPD_Peds[1][channel - 1];
    width = RPD_PedWidths[1][channel - 1];
    gain = RPD_Gains[1][channel - 1];
  }
  energy = subPedestal(charge(digi, signalTs_), ped, width) * gain;
  return (energy);
}
...
double ZDCHardCodeHelper::rechit_Time(const QIE10DataFrame& digi) {
  ...
  if (signalTs_ > 0 && signalTs_ < digi.samples() - 1) {
    ...
    if (total_energy > 0)
      time = 25.0 * (weight_energy / total_energy);
  }
  return (time);
}
