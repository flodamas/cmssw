...
private:
  ...
  std::pair<unsigned int, const pat::CompositeCandidate*> theBestQQ();
  ...
  void fillRecoHistos();
  void fillRecoJpsi(int count, std::string centName);
  void fillHistosAndDS(unsigned int theCat, const pat::CompositeCandidate* aJpsiCand);

  void fillTreeMuon(const pat::Muon* muon, int iType);
  ...
  float Reco_mu_normChi2[Max_mu_size];
  ...
  if (muon != NULL) {
    ...
    if (!_theMinimumFlag) {
      ...
      if (iTrack.isNonnull()) {
        ...
        Reco_mu_normChi2[Reco_mu_size] = (muon->isGlobalMuon()) ? muon->globalTrack()->normalizedChi2() : iTrack->normalizedChi2();
        ...
        Reco_mu_nMuValHits[Reco_mu_size] = (muon->isGlobalMuon()) ? muon->globalTrack()->hitPattern().numberOfValidMuonHits() : iTrack->hitPattern().numberOfValidMuonHits();
...
void HiOniaAnalyzer::fillRecoJpsi(int count, std::string centName) {
...
bool HiOniaAnalyzer::checkCuts(const pat::CompositeCandidate* cand,
                               ...
                               bool (HiOniaAnalyzer::*callFunc2)(const pat::Muon*)) {
  if ( (
       ((this->*callFunc1)(muon1) && (this->*callFunc2)(muon2)) ||
